...
def get_3D_overlaps_blocks(qG, k2: np.ndarray, blocks: dict, N_AO: int, mo_coeff_i: np.ndarray, mo_coeff_f_conj: np.ndarray, unique_Ri: list[np.ndarray], q_cuts: np.ndarray) -> np.ndarray:
    """
    Notes:
    ...
    Inputs:
        ...
        k2:             np.ndarray of shape (N_kpairs, 3): k' corresponding to q
...
def RPA_susceptibility(eta_qG, re_delE, im_delE, N_kpairs, V_cell):
    """
    Notes: 
    - Weird im(chi) part may be due to chi_num calculation, changing to np.abs instead of np.conj for non-lfe case to see if this fixes issue. This function only works for non-lfe now!
    ...
    Inputs:
        ...
        V_cell:     float: volume of unit cell

    Outputs:
        chi:        np.ndarray of shape (N_enegies,): Kohn-Sham susceptibility, Eq.() 
    """
    #chi_num = np.conjugate(eta_qGp)*eta_qG #(k_pair,i,j)
