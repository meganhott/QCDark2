...
def RPA_susceptibility(E, G_id, Gp_id, k_pairs, mo_en_i, mo_en_f, eta_q):
    ...
    Inputs:
        E:          np.array of shape (N_energies,): all energies
        G_id:       int: index of G vector from G_vectors
        Gp_id:      int: index of G' vector from G_vectors
        k_pairs:    np.array of shape (N_k_pairs, 2): (ki_id, kf_id) pairs
        mo_en_i:    np.array of shape (N_ki, N_val_bands, N_AO): initial molecular orbital energies
        mo_en_f:    np.array of shape (N_kf, N_con_bands, N_AO): final molecular orbital energies
        eta_q:      dict: all 3D overlaps for q generated by get_3D_overlaps
