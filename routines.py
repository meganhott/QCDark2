...
def RPA_dielectric(G, q, k_f, mo_coeff_i, mo_coeff_f_conj, re_delE, im_delE, R_id, unique_Ri, blocks, N_AO, N_kpairs, V_cell):
    """
    To Do:
...
def initialize_RPA_dielectric(dark_objects, test=False):
    ...
    for q in unique_q.keys():
        ...
        with mp.get_context('fork').Pool(mp.cpu_count()) as p:  #parallelization over G
            eps = p.map(partial(RPA_eps, q=q, k_f=k_f_q, mo_coeff_i=mo_coeff_i_q, mo_coeff_f_conj=mo_coeff_f_q_conj, re_delE=re_delE, im_delE=im_delE, R_id=R_id, unique_Ri=unique_Ri, blocks=blocks, N_AO=N_AO, N_kpairs=N_kpairs, V_cell=V_cell), G_q)
