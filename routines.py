...
def RPA_susceptibility(E, eta_qG, eta_qGp, mo_en_i, mo_en_f):
    ...
    """
    delE = mo_en_f[:,None,:] - mo_en_i[:,:,None] #(k_pair,i,j)
...
def RPA_dielectric(G, q, mo_en_i, mo_en_f, k_f, mo_coeff_i, mo_coeff_f_conj, R_id, unique_Ri, blocks, N_AO):
    """
    To Do:
    ...
    eps = 1 - (4*np.pi)**2 / (np.dot(q,q)+np.dot(G,G)) * RPA_susceptibility(E, eta_qG, eta_qG, mo_en_i, mo_en_f)
...
def initialize_RPA_dielectric(dark_objects):
    ...
    for q in unique_q.keys():
        ...
        with mp.get_context('fork').Pool(mp.cpu_count()) as p:  #parallelization over G
            eps = p.map(partial(RPA_eps, q=q, mo_en_i=mo_en_i_q, mo_en_f=mo_en_f_q, k_f=k_f_q, mo_coeff_i=mo_coeff_i_q, mo_coeff_f_conj=mo_coeff_f_q_conj, R_id=R_id, unique_Ri=unique_Ri), G_q)
