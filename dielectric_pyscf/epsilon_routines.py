...
def get_RPA_dielectric(dark_objects: dict):
    if parmt.include_lfe:
        get_RPA_dielectric_LFE_new(dark_objects)
    else:
...
def get_RPA_dielectric_LFE_new(dark_objects: dict):
    # Reading all relevant data
    N_AO = len(dark_objects['aos'])
    ivalbot, ivaltop, iconbot, icontop = np.load(parmt.store + '/bands.npy')
    ...
    for i_q, q in enumerate(q_keys, start=q_start):
        ...
        eps_delta_q = get_RPA_dielectric_LFE_q_new(q, mo_en_f, mo_en_i, mo_coeff_f_conj, mo_coeff_i, k_f, k_pairs, primgauss_arr, AO_arr, coeff_arr, q_cuts, VCell, G_q, unique_Ri, einsum_path) #(G,G',E)
...
@time_wrapper(n_tabs=2)
def get_RPA_dielectric_LFE_q_new(q: np.ndarray, mo_en_f: np.ndarray, mo_en_i: np.ndarray, mo_coeff_f_conj: np.ndarray, mo_coeff_i: np.ndarray, k_f: np.ndarray, k_pairs: np.ndarray, primgauss_arr, AO_arr, coeff_arr, q_cuts: np.ndarray, VCell: float, G_q: np.ndarray, unique_Ri: list[np.ndarray], einsum_path) -> np.ndarray:
    ...
    epsilon_delta = prefactor*RPA_Im_eps_external_prefactor_LFE_new(qG, primgauss_arr, AO_arr, coeff_arr, q_cuts, unique_Ri, einsum_path)
    
    return epsilon_delta


def RPA_Im_eps_external_prefactor_LFE_new(qG, primgauss_arr, AO_arr, coeff_arr, q_cuts, unique_Ri, einsum_path):
    """
    Returns delta function part of lim_{n->0} |<j(k+q)|exp(i(q+G)r)|ik>|^2/|q+G|^2/(E - (E_{j(k+q)} - E_{ik} + i n)), summed over i,j,k. 
